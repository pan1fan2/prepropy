...
def test_pairplot():
    ...
    eda_res = eda.eda(test_data,"target")
    p = eda_res["pairplot"]
    assert isinstance(p, alt.vegalite.v4.api.RepeatChart), 'Wrong output for altair plot'
    
def test_eda_raise_error_not_dataframe():
    test_input = [1, 2, 3, 4, 5]

    with pytest.raises(TypeError, match="Input data must be instance of pandas.core.frame.DataFrame."):
