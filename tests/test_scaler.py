...
X_test_non_num = pd.DataFrame(np.array([['adam', 54, 500], ['eve', 'Iamnotnum', 6000],['pandaman', 64, 9000]]),
...
scaled_max_abs_X_train = np.array([[-0.04295367795875608, -1.3258384266690595],
       [-1.202702982845162, 0.2367568619051891],
       [1.2456566608039172, 1.08908156476387]])

scaled_max_abs_X_test = np.array([[-1.202702982845162, 18.41968385622372],
       [-4.037645728123043, 14.442168576216542],
       [-5.455117100761984, 21.544874433372215]])

scaled_max_abs_X_Valid =np.array([[-0.04295367795875608, 3.646055673339913],
...
def test_scaled_values():
    ...
    assert np.array_equal(temp['X_test'].drop(columns=['name']).to_numpy(), scaled_max_abs_X_test), 'Incorrect scaled test values'
    assert np.array_equal(temp['X_train'].drop(columns=['name']).to_numpy(), scaled_max_abs_X_train), 'Incorrect scaled train values'
    assert np.array_equal(temp['X_Valid'].drop(columns=['name']).to_numpy(), scaled_max_abs_X_Valid), 'Incorrect scaled Valid values'
...
def test_scaling_method():
    ...
    with pytest.raises(KeyError):
        scaler(X_train, X_Valid, X_test, ['age', 'net_worth'], scaler_type="spacescaler")
