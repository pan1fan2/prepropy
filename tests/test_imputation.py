...
def test_imputation():
    ...
    df2_t = pd.DataFrame([[ 4.,  2.,  3.],[ 4.,  5.,  6.],[10.,  5.,  9.],[ 3., 15., 17.]])
    ...
    assert np.array_equal(df1_filled.to_numpy(), df1_t.to_numpy()), 'Generated Output are incorrect'
    
    # test for median imputation
    imputer = imputation('median')
    imputer.fit(df2)
    df2_filled = imputer.fill(df2)
    assert np.array_equal(df2_filled.to_numpy(), df2_t.to_numpy()), 'Generated Output are incorrect'
    
    # test for most frequent imputation
    imputer = imputation('most_frequent')
    imputer.fit(df3)
    df3_filled = imputer.fill(df3)
    assert np.array_equal(df3_filled.to_numpy(), df3_t.to_numpy()), 'Generated Output are incorrect'
    
    # test for dataframe with non nan values
    imputer = imputation('mean')
    imputer.fit(df1_t)
    df4_filled = imputer.fill(df1_t)
    assert np.array_equal(df4_filled.to_numpy(), df1_t.to_numpy()), 'Generated Output are incorrect'
    
    # test for mix type dataframe for most_frequent method
    imputer = imputation('most_frequent')
    imputer.fit(df_m)
    df5_filled = imputer.fill(df_m)
    assert np.array_equal(df5_filled.to_numpy(), df_m_t.to_numpy()), 'Generated Output are incorrect'
    
    # test for dataframe where there is only a single value
    imputer = imputation('mean')
    imputer.fit(df_s)
    df6_filled = imputer.fill(df_s)
    assert np.array_equal(df6_filled.to_numpy(), df_s.to_numpy()), 'Generated Output are incorrect'
